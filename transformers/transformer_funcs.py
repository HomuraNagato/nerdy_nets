...
def Self_Attention(K, V, Q, use_mask=False):
    ...
    intermediate = tf.nn.softmax(intermediate)
    attention = tf.matmul(intermediate, V)
...
class Multi_Headed(tf.keras.layers.Layer):
    def __init__(self, emb_sz, use_mask):
        ...
        # TODO:
	# Initialize heads
        self.cerberus_1 = Atten_Head(emb_sz, int(emb_sz/3), use_mask)
        self.cerberus_2 = Atten_Head(emb_sz, int(emb_sz/3), use_mask)
        self.cerberus_3 = Atten_Head(emb_sz, int(emb_sz/3), use_mask)
        self.W = self.add_weight("W",shape=[emb_sz, emb_sz])
    ...
    def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
        ...
        :return: tensor of [BATCH_SIZE x (ENG/FRN)_WINDOW_SIZE x output_size ]
        """
        atten1 = self.cerberus_1(inputs_for_keys, inputs_for_values, inputs_for_queries)
        atten2 = self.cerberus_2(inputs_for_keys, inputs_for_values, inputs_for_queries)
        atten3 = self.cerberus_3(inputs_for_keys, inputs_for_values, inputs_for_queries)

        attention = tf.concat([atten1, atten2, atten3], 2)

        #print("dim of attention:", attention.shape, "self.W shape:", self.W.shape)
        
        attention = tf.tensordot(attention, self.W, [2,0])

        #print("dim of attention dotted with weight:", attention.shape)
